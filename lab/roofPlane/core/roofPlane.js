...
import { resolveValue } from '../../../src/core/projectParams.js';
import { computeSlopeFromClearance } from '../../../src/core/trussLayout.js';
...
import { findRoofObstructions, applyObstructionsToRun } from '../../../src/core/roofObstructions.js';
...
export function resolveRoofPlane({ model, plane, paramsMap = {}, elementsById = {}, library = null } = {}) {
  ...
  for (const { wallId, line } of highLines) {
    ...
    tramos.push({ wallHighId: wallId, runFrom: tFrom, runTo: tTo, span, perpHigh });
  }
  ...
  const purlinH = plane.purlinProfileH ?? 0; // altura del perfil de costanera (para holgura)
  ...
  const purlinsRes = buildRoofPurlins({
    ...
    spacing: resolveValue(plane.purlinSpacing ?? 600, paramsMap, elementsById),
  ...
  return {
    resolved: true, runAxis, perp: perpCanal, supportElevation, slopePercent,
    supportLine,
    tramos: tramos.map(t => ({
      wallHighId: t.wallHighId, runFrom: t.runFrom, runTo: t.runTo,
      span: t.span, inclSpan: t.inclSpan, hiddenBy: t.hiddenBy
    })),
